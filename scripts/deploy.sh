...
if [ -z "$DEPLOY_HOST" ] || [ -z "$DEPLOY_USER" ] || [ -z "$DEPLOY_PASS" ] || [ -z "$DEPLOY_PATH" ]; then
  echo "❌ .env.deploy 파일에 DEPLOY_HOST, DEPLOY_USER, DEPLOY_PASS, DEPLOY_PATH를 설정하세요."
  exit 1
fi
...
sshpass -e rsync -avz -e "ssh -o StrictHostKeyChecking=no" \
  --exclude='node_modules' --exclude='.git' --exclude='data' --exclude='.env.deploy' \
  "$PROJECT_DIR/" "$DEPLOY_USER@$DEPLOY_HOST:$DEPLOY_PATH/"

echo "🔄 서버에서 빌드 + 재시작..."
sshpass -e ssh -o StrictHostKeyChecking=no "$DEPLOY_USER@$DEPLOY_HOST" \
  "cd $DEPLOY_PATH/client && npm install && npm run build && pm2 restart rnc-voca"
